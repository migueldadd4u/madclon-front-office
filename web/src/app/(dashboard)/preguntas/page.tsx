...
const PreguntasPage = () => {
  ...
  const { data, faltantes } = usePanelData()

  // Rellena los huecos con los JSON. Si alguno de los tres documentos falta o no
  // validó, la respuesta usa su variante sin cifras: mejor decir dónde mirar
  // que enseñar un número muerto.
  const vivo = (p: { a: string; sinDatos?: string }) => {
    if (!p.sinDatos) return p.a

    const faltan = (['tokens', 'overview', 'clones'] as const).some(d => faltantes.includes(d) || !data[d])

    if (faltan) return p.sinDatos
